...
if __name__ == "__main__":
    ...
    if os.path.basename(os.path.normpath(file)) == "pycalib":
        ...
        file = os.path.split(os.path.split(file)[0])[0] + "/pycalib/datasets/mnist/"
    ...
    for ns in tqdm(n_samples):
        ...
        # Run
        mnist_benchmark.run_kernel(n_jobs=1)
    ...
    for reg in tqdm(reg_candidates):
        ...
        # Run
        mnist_benchmark.run_kernel(n_jobs=1, reg_sensitive=True)
        scores = pd.concat([scores, mnist_benchmark.results], axis=0)
    scores.to_csv(path_or_buf=os.path.join(mnist_benchmark.run_dir, "cv_scores_total_reg_{}.csv".format(time.strftime("%Y%m%d-%Hh%Mm%Ss"))))