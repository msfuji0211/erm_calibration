...
class Benchmark(object):
    ...
    def run(self, n_jobs=None, train=False):
        """
        ...
        for cal_method, cal_method_name in zip(self.cal_methods, self.cal_method_names):
            ...
            for X, y, data_params in self.data_gen_tr() if train else self.data_gen():
                ...
                metrics_dict = {
                    ...
                    #"ECE_opt": (sc.expected_calibration_error, {"p": 1, "n_bins": int((data_params["calib_size"])**(1/3))}), ## Adding this: our optimul number of bins.
                    "ECE_opt": (sc.ece_ours, {"n_bins": int((data_params["calib_size"])**(1/3))}), ## Adding this: our optimul number of bins.
                    "ECE_opt_test": (sc.ece_ours, {"n_bins": int((self.test_size)**(1/3))}), ## Adding this: our optimul number of bins.
                    ...
                    "KCE_gauss": (sc.kernel_calibration_error, {}),
                    "KCE_laplace": (sc.kernel_calibration_error, {"kernel_type": "laplace"}),
                    "smoothCE": (sc.compute_LinECE, {}),
                }
                if len(labels) == 2:
                ...
                print({k: len(v) for k, v in model_scores_dict.items()})
                ...
                model_scores = pd.concat([model_scores, model_scores_tmp], ignore_index=True)

                # Save summary statistics
                grouped = model_scores_tmp.groupby(list(info_params.keys()))
                summ_stats_model_scores = grouped.agg([np.mean, np.std]).reset_index()
                #scores = scores.append(summ_stats_model_scores)
                scores = pd.concat([scores, summ_stats_model_scores], ignore_index=True)
                ...
                #    h, m, s
                #))
                print("{} {}-fold results:\t ECE (opt): {:.4f} +/- {:.4f}\t Time: {:.0f}:{:02.0f}:{:02.0f}".format(
                    cal_method_name,
                    self.cross_validator.n_splits,
                    np.mean(model_scores_dict["ECE_opt"]),
                    np.std(model_scores_dict["ECE_opt"]),
                    h, m, s
                ))
                print("{} {}-fold results:\t KCE (Gaussian): {:.4f} +/- {:.4f}\t Time: {:.0f}:{:02.0f}:{:02.0f}".format(
                    cal_method_name,
                    self.cross_validator.n_splits,
                    np.mean(model_scores_dict["KCE_gauss"]),
                    np.std(model_scores_dict["KCE_gauss"]),
                    h, m, s
                ))
                print("{} {}-fold results:\t KCE (Laplace): {:.4f} +/- {:.4f}\t Time: {:.0f}:{:02.0f}:{:02.0f}".format(
                    cal_method_name,
                    self.cross_validator.n_splits,
                    np.mean(model_scores_dict["KCE_laplace"]),
                    np.std(model_scores_dict["KCE_laplace"]),
                    h, m, s
                ))
                print("{} {}-fold results:\t smoothCE: {:.4f} +/- {:.4f}\t Time: {:.0f}:{:02.0f}:{:02.0f}".format(
                    cal_method_name,
                    self.cross_validator.n_splits,
                    np.mean(model_scores_dict["smoothCE"]),
                    np.std(model_scores_dict["smoothCE"]),
                    h, m, s
                ))
            ...
            if train:
                model_scores.to_csv(
                    path_or_buf=os.path.join(method_dir, "cv_scores_tr_{}_{}.csv".format(cal_method_name, tmp_time)))
            else:
                model_scores.to_csv(
            ...
            if train:
                scores.to_csv(
                    path_or_buf=os.path.join(method_dir, "cv_scores_tr_{}_{}_summ.csv".format(cal_method_name, tmp_time)))
            else:
                scores.to_csv(
        ...
        if train:
            scores.to_csv(
                path_or_buf=os.path.join(self.run_dir, "cv_scores_total_tr_{}.csv".format(time.strftime("%Y%m%d-%Hh%Mm%Ss"))))
        else:
            scores.to_csv(
                path_or_buf=os.path.join(self.run_dir, "cv_scores_total_{}_{}_{}.csv".format(self.train_size, self.test_size, time.strftime("%Y%m%d-%Hh%Mm%Ss"))))

        self.results = scores
    
    def run_kernel(self, n_jobs=None, train=False, reg_sensitive=False):
        """
        Train all models, evaluate on test data and save the results.
        ...
        for cal_method, cal_method_name in zip(self.cal_methods, self.cal_method_names):
            ...
            for X, y, data_params in self.data_gen_tr() if train else self.data_gen():
                ...
                # Convert result to data frame and add to cal_method scores data frame
                model_scores_tmp = pd.DataFrame(model_scores_dict)
                #info_params = {**{"Method": cal_method_name, "alpha": cal_method.alpha}, **data_params}
                info_params = {
                    "Method": cal_method_name,
                    "alpha": getattr(cal_method, "alpha", None),
                    **data_params
                }
                model_scores_tmp = model_scores_tmp.assign(**info_params)
                #model_scores = model_scores.append(model_scores_tmp)
        ...
        if train:
            if reg_sensitive:
                pass
                #scores.to_csv(
                #path_or_buf=os.path.join(self.run_dir, "cv_scores_total_tr_kernel_reg_{}.csv".format(time.strftime("%Y%m%d-%Hh%Mm%Ss"))))
            else:
                scores.to_csv(
                path_or_buf=os.path.join(self.run_dir, "cv_scores_total_tr_kernel_{}.csv".format(time.strftime("%Y%m%d-%Hh%Mm%Ss"))))
        else:
            if reg_sensitive:
                pass
                #scores.to_csv(
                #    path_or_buf=os.path.join(self.run_dir, "cv_scores_total_kernel_reg_{}_{}_{}.csv".format(self.train_size, self.test_size, time.strftime("%Y%m%d-%Hh%Mm%Ss"))))
            else:
                scores.to_csv(
                path_or_buf=os.path.join(self.run_dir, "cv_scores_total_kernel_{}_{}_{}.csv".format(self.train_size, self.test_size, time.strftime("%Y%m%d-%Hh%Mm%Ss"))))

        self.results = scores
    
    
    def run_kl(self, n_jobs=None, train=False):
        """
        Train all models, evaluate on test data and save the results.
    ...
    def run_lr(self, n_jobs=None, train=False):
        """
        Train all models, evaluate on test data and save the results.
        ...
        for cal_method, cal_method_name in zip(self.cal_methods, self.cal_method_names):
            ...
            for X, y, data_params in self.data_gen_tr() if train else self.data_gen():
                ...
                # Convert result to data frame and add to cal_method scores data frame
                model_scores_tmp = pd.DataFrame(model_scores_dict)
                info_params = {**{"Method": cal_method_name, "lr": cal_method.learning_rate, "alpha": cal_method.alpha}, **data_params}
                model_scores_tmp = model_scores_tmp.assign(**info_params)
                model_scores = model_scores.append(model_scores_tmp)
...
class KITTIBinaryData(Benchmark):
    ...
    def classify_val_data(file, clf_name, classifier, data_folder='kitti', output_folder='clf_output'):
        ...
        p_pred_tr = classifier.predict_proba(X_train)
        y_y_pred_tr = np.column_stack([y_train, np.argmax(p_pred_tr, axis=1)])

        # Remove possible nan or inf
        isnotnan_ind = ~np.isnan(p_pred_tr).any(axis=1)
        p_pred_tr = p_pred_tr[isnotnan_ind, :]
        y_y_pred_tr = y_y_pred_tr[isnotnan_ind, :]
        ...
        np.savetxt(os.path.join(out_path, "p_pred_tr_{}.csv".format(clf_name)), p_pred_tr, delimiter=",")
        np.savetxt(os.path.join(out_path, "y_y_pred_tr_{}.csv".format(clf_name)), y_y_pred_tr, fmt='%i', delimiter=",")
    ...
    def data_gen_tr(self):
        """
        Returns the full dataset or a generator of training datasets.

        Returns
        -------
            X, y giving uncalibrated predictions and corresponding classes on training dataset.
        """
        # Load logits and classification results from file
        for clf_name in self.classifier_names:
            X = np.loadtxt(os.path.join(self.clf_output_dir, 'p_pred_tr_' + clf_name + '.csv'), dtype=float, delimiter=',')
            y = np.loadtxt(os.path.join(self.clf_output_dir, 'y_y_pred_tr_' + clf_name + '.csv'),
                           dtype=int, delimiter=',')[:, 0]
